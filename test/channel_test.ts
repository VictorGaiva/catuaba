...
import { PhoenixChannel, PhoenixSocket } from '../src/';
import { MessageFromSocket, MessageToSocket } from '../src/phoenix/types';
...
describe('with transport', () => {
  ...
  describe('join', () => {
    let socket: PhoenixSocket<MessageToSocket<unknown>, MessageFromSocket<unknown>>;
    let channel: PhoenixChannel;
    beforeEach(() => {
      socket = new PhoenixSocket('/socket');
      channel = new PhoenixChannel('test', {}, socket);
    });
    ...
    it('throws if attempting to join multiple times', () => {
      ...
      expect(() => channel.join()).toThrow(/^Error: tried to join multiple times/);
    });
    ...
    it('triggers socket push with channel params', () => {
      ...
      channel = new PhoenixChannel('test', { one: 'two' }, mock as any);
  ...
  describe('joinPush', () => {
    ...
    beforeEach(() => {
      ...
      socket = new PhoenixSocket('/socket', { timeout: defaultTimeout });
      sinon.stub(socket, 'isConnected').callsFake(() => true);
  ...
  describe('leave', () => {
    ...
    // TODO - the following tests are skipped until PhoenixChannel.leave
    // behavior can be fixed; currently, 'ok' is triggered immediately
    // within PhoenixChannel.leave so timeout callbacks are never reached
    //
